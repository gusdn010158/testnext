...
// "use client";
// import React, { useState, useEffect } from "react";
// import useApi from "@/services/api"; // api를 useApi로 수정
// import styles from "./Detail.module.css";
// import { BsBoxSeamFill } from "react-icons/bs";
...
// function page(props) {
//   const url = "http://localhost:3001/test";
//   const test = useApi(url); // api -> useApi로 변경
//   const [index, setIndex] = useState(0);
//   const [open, setOpen] = useState(null);
...
// export default page; // 컴포넌트 이름을 대문자로 변경
"use client";
import React, { useState, useEffect } from "react";
import useApi from "@/services/api"; // api를 useApi로 수정
import styles from "./Detail.module.css";
            ...
            <div className={styles.dleftimg}>
              {selectedTestData &&
                selectedTestData.images
                  .filter((item) => index === item.index)
                  .map((imageObj) => (
                    <img src={imageObj.img} key={imageObj.index} /> // key 추가
                  ))}
            </div>
          </div>
        </div>
